...
////////////////////////////////////////////////////////////////////
//
//  ALTERA_ONCHIP_FLASH_AVMM_DATA_CONTROLLER (SERIAL-to-SERIAL MODE)
//
...
module altera_onchip_flash_avmm_data_controller (
    ...
    generate
        if (READ_AND_WRITE_MODE == 1) begin
            ...
            assign is_valid_write_burst_count = (avmm_burstcount == 32);
        end
        else begin
    ...
    assign avmm_waitrequest = (~reset_n || ((op_state == OP_STATE_IDLE || op_state == OP_STATE_RESET || op_state == OP_STATE_CLEAR || op_wait_wire) && (avmm_write || avmm_read)) || (avmm_read && ~is_busy_read));
    assign avmm_readdatavalid = avmm_readdatavalid_neg_reg;
    assign avmm_readdata = (csr_status_r_pass) ? flash_drdout_neg_reg : 1'b1;
                    ...
                    case (op_state)
                        ...
                        OP_STATE_ADDR: begin
                            ...
                            if (counter_count_wire == (FLASH_ADDR_WIDTH-1)) begin
                                ...
                                arclk_en <= 0;
                                op_count_start <= 1;
                                if (~is_busy_erase) begin
                                    op_wait <= 0; // clear waitrequest signal to allow master continue shiftin the writedata
                                end
                        ...
                        OP_STATE_READ_DATA: begin
                            ...
                            if (cur_burstcount == 0) begin
                                ...
                                if (counter_count_wire == 30) begin
                                    avmm_readdatavalid_reg <= 0;
                                    enable_arclk_neg_pos_reg <= 0;
                                    if (op_wrap_back_addr || op_range2_addr || op_range3_addr) begin
                                        op_count_start <= 1;
                                        op_state <= OP_STATE_ADDR;
                                    end
                                    else begin
